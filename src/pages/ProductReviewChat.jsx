...
const ProductReviewChat = () => {
  ...
  const generatePDF = () => {
    ...
    // Create a hidden container to render the markdown-styled content
    ...
    <div style="text-align:center; border-bottom: 2px solid #007bff; padding-bottom:10px; margin-bottom:20px;">
      <h1 style="color:#007bff; margin:0; font-size:22px;">🔍 Product Review Report</h1>
      <p style="margin:4px 0; color:#666;">Generated by AI Product Reviewer</p>
      <p style="margin:4px 0; color:#666;">Analysis Date: ${new Date().toLocaleDateString()}</p>
    </div>
  `;

    // ✅ Add AI response converted from markdown → styled HTML
    const analysisSection = document.createElement("div");
    analysisSection.innerHTML = `
    <h2 style="color:#007bff; font-size:18px;">📊 Product Analysis</h2>
    <div style="font-size:14px; line-height:1.7; margin-top:10px;">
      ${lastAnalysis.analysisText
        // Bold → yellow
        .replace(/\*\*(.*?)\*\*/g, '<span style="color:#eab308; font-weight:bold;">$1</span>')
  ...
  `;
    ...
    html2canvas(container, { scale: 2 }).then((canvas) => {
      ...
      pdf.save(
        `${lastAnalysis.productName.replace(/\s+/g, "_")}_Review_Report.pdf`
      );
